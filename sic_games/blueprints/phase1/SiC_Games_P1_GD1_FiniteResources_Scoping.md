...
**Status:** **BUILT (opt-in `enable_depletion=True`; default OFF ⇒ non-depleting standing flow, bit-exact) 2026-07-03.** `capacity.py::NPPCapacityField` holds a depletable stock `B∈[0.05,1]` per cell of its ceiling, biome- + aquatic-specific logistic regrowth (`R_BIOME_PER_YR`/`AQUATIC_R_PER_YR`) minus foraging pressure, advanced by `deplete_and_regrow(occ_count, season)`; hooked in `phase1_model.py::_step_rivalrous`. Built + viable + lit-anchored (Coe 1976 / Cortés 2016). See RESULTS R-51; MODEL_SPEC §4.3.11; MECHANISMS §9b.9; PARAMETERS §19.7. **Biome/season regrowth rates PROVISIONAL (sweep + sign-off pending).** **Finding (R-51):** depletion is necessary but NOT sufficient for emergent sedentism — the population is demographically (not resource) limited, so Carneiro circumscription/saturation is the missing keystone. Original scoping retained below.
