...
class ReproductionCoordinator:
    ...
    def _carbon_birth(
        ...
        # Stage 6.0a: spatial_hash is pos → LIST of agents (multi-occupancy). At K_cell=1
        # lists are singletons → identical to the legacy single-agent hash (recovery-safe).
        _sub = getattr(world, "_substrate_cfg", None)
        _diffusion = bool(_sub is not None and _sub.enabled and _sub.movement_mode == "diffusion")
        if _diffusion and spatial_hash is not None:
            # §2.3: parents must be CO-LOCATED (same cell, the band).
            candidates = [
                a for a in spatial_hash.get(agent.pos, [])
                if a is not agent and a.strategy == "carbon"
            ]
        elif spatial_hash is not None:
            # Legacy/recovery: Chebyshev radius-r scan over the pos→list hash.
            candidates = []
            for dr in range(-r, r + 1):
                for dc in range(-r, r + 1):
                    if max(abs(dr), abs(dc)) <= r:   # Chebyshev condition
                        ...
                        for neighbour in spatial_hash.get((nr, nc), ()):
                            if neighbour is not agent and neighbour.strategy == "carbon":
        ...
        # Stage 6.0a §2.3: in diffusion (production) mode, offspring spawns ON the parent's
        # cell (co-occupancy). Legacy/recovery keeps the standard placement (recovery-safe).
        if _diffusion:
            offspring = world._spawn_one(traits=mixed, exact_pos=agent.pos)
        else:
