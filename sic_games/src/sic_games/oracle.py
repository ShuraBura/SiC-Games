"""oracle.py — D4 Archived Oracle (Stage 7.5, 2026-06-08).

This module is the authoritative home of SugarWorld, the original sequential
object-model oracle.  It was moved here from run.py after GATE FINAL passed
(all 5 seeds: exact N_final + dormancy match across SugarWorld vs SoAWorld).
SoAWorld (soa_step.py) is now the production model; SugarWorld is retained
as the reference implementation for parity tests and future regression checks.

D4 decision (ARCHITECTURE.md §H.6): "keep the object model frozen as the
reference oracle until FINAL gate passes + a known science result reproduces;
then → archive/, not deleted." This module IS the archive home.

Import from here:  from sic_games.oracle import SugarWorld
Backward-compat:   from sic_games.run import SugarWorld  (re-exports this)

FROZEN: do not add new features here.  Bugs that affect the oracle's
correctness as a reference should be fixed; all other changes go to SoAWorld.
"""
...
class SugarWorld(mesa.Model):
    """Sugarscape model — Stage 1 through Stage 7.5 (D4-frozen oracle).

    D4 status: ARCHIVED 2026-06-08.  SoAWorld (soa_step.py) is the production
    model.  This class is retained as the reference oracle for parity tests.
    GATE FINAL confirmed exact match (all 5 seeds, 400 steps) on the Stage 5.1
    seasonal science config.  Do not add new features here.
