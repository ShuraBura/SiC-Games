...
# Sex-specific + maternal-removed female fit (blueprint M-3) is pending Hill & Hurtado's
# sex-specific Aché tables; Step 1 applies the both-sexes schedule to both sexes and adds an
# explicit per-birth maternal hazard (so maternal deaths are NOT double-counted at lock).
...
class DemographyConfig(BaseModel):
    ...
    maternal_mortality_per_birth: float = Field(0.012, ge=0.0, le=1.0)  # per-birth [verify Aché]
    ...
    def siler(self) -> SilerParams:
        return SilerParams(self.siler_a1, self.siler_b1, self.siler_a2,
                           self.siler_a3, self.siler_b3)
