...
| Aquatic-gated MORPH | storage stays a broad survival buffer; the surplus→COMPLEX morph is gated on the band's mean water access (`wateracc`) → complexity requires a storable aquatic resource (NW-Coast salmon); terrestrial interiors egalitarian+surviving. The correct forager-complexity pattern | C2 | `phase1_model` per-band morph detector (mean `wateracc` gate) | §4.5.10; R-47 |
