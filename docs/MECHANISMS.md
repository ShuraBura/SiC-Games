...
**Finding (R-52):** the chain closes — SETTLE holds 2–3 persistent settlements (~110), band_dens 0.19–0.34 (2–4× packing), %packed 38–75%, morph→complex, population survives+grows, vs 0% packed OFF. Anchors: Mauss & Beuchat 1979, Binford 2001 (collector/packing), Vita-Finzi & Higgs 1970 (catchment), Johnson 1982 (scalar stress), Testart 1982, Ames 1994. Default OFF ⇒ bit-exact. Params §20.

**Layer 2b — SHOCK (R-53).** A regional bad-year multiplier on tier-2 yield: once per year a mean-preserving lognormal, **AR(1)** in `shock_rho` (0 = IID single years, →1 = multi-year ENSO/PDO regimes). Storage (`_cell_store`) buffers it → dispersal emerges on deficit via the existing starvation + settlement-dissolve. **Finding:** fisheries are STABLE — storage fully buffers even harsh regimes (0 dissolutions / 4 seeds; a village rode out a shock-0.21 multi-year regime) = the NW-Coast benchmark. The shock instead halves the *mobile hinterland* population while the storing village is insulated. So the acute dynastic **bust is an AGRICULTURE (soil-depletion) phenomenon**, not a fishery one → the cultivability tier (blueprint §10). **OPEN:** agriculture tier (soil-depletion + landesque learning → the bust); "complex" not yet "stratified".
...
| Productivity-scaled mobility | diffusion STRIDE scales ∝1/static-local-NPP (Kelly/Binford); water-aware glide; low-NPP agents spread instead of piling → fixes the biome→society collapse root | C1 | `demography::mobility_radius`; `substrate::diffusion_select_target(move_radius,water)` | §4.8.19; R-39 (FALSIFIED as fix, DE-9) |
| Central-place co-movement (footprint) | family CO-RESIDES but forages DISPERSED (Isaac 1978); footprint scatters followers off the mother's cell → kills the over-subscription → resolves the biome collapse. Also anticipate / provision-exclude variants | C1 | `substrate::diffusion_select_target(extra_occupants)`; `phase1_model` footprint scatter + `_forage_excl` | §4.8.20; R-41/R-42 |
| Storability-gated morph (seasonality) | gate the STORE on biome seasonal amplitude — forest egalitarian but mis-orders desert (complex). SUPERSEDED by the aquatic morph gate | C2 | `climate::seasonal_amplitude_field`; `phase1_model` `in_owz` | §4.5.10; R-46 |
| Aquatic-gated MORPH (final) | storage stays a broad survival buffer; a band morphs COMPLEX only if BOTH seasonal aquatic glut `mean(wateracc×seas_amp)≥thr` AND productive setting `mean(npp_gm2)≥floor` — the salmon-river/Nile signature. Forest (aseasonal) + desert (poor) egalitarian; seasonal-productive rivers complex. Distinguishes true-desert from river-desert | C2 | `phase1_model` per-band morph detector (glut × npp-floor gate) | §4.5.10; R-47/R-48 |
