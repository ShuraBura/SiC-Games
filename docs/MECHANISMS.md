...
| Storability-gated morph | gate the overwintering STORE on biome SEASONAL AMPLITUDE (Testart/Binford) → aseasonal forest can't store → egalitarian; seasonal biomes store → surplus → complex. Makes the morph fit the biome | C2 | `climate::seasonal_amplitude_field`, `BIOME_SEASONAL_AMP_BY_CODE`; `phase1_model` `in_owz` gate | §4.5.10; R-46 |
