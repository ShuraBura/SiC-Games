...
**Last updated:** 2026-07-04 (EFC C1–C8 + GD-1 finite-resources arc documented — see the active-line table below).
...
> **2026-07-21 (R-97) — the prerequisite recorded on this line was correct, and was proceeded past.** The
> elite/legitimacy layer (R-82…R-96) was built at VILLAGE scale and works, but produces no secular cycles: four
> arms below the calibrated noise floor. Villages revolt on independent clocks and uncoupled oscillators sum to
> a flat aggregate. Turchin's cycles need the superordinate polity this line already named as a prerequisite —
> taxation, standing armies, elite competition for finite offices. **The next legitimate attempt at cycles is
> the supra-village rung, not another village-scale mechanism.** See RESULTS R-97, DEAD_ENDS DE-14 revision.
...
| **Economy-from-Climate (EFC) C1–C8 + GD-1 finite resources** | ✓ **built (opt-in mode; legacy default bit-exact)** | The deep first-principles substrate (supervisor: "B is too tempting") — food yield EMERGES from climate. `generate_world(…, mode="climate")`: **C1** temperature (latitude − 6.5 °C/km lapse + seasonal amp), **C2** precipitation (Hadley/ITCZ × orographic rain-shadow × maritime × aridity), **C3** NPP = **Miami(T,P)** (Lieth 1972/1975, **VERIFIED** vs the primary PDF eqs 12-1/12-2), **C4** biome = **Whittaker(T,P)** + terrain×climate world-lottery, **C6** river-source (headwater) water temperature, **C7** aquatic-food (salmon + shellfish), **C8** aquatic capacity subsidy. **GD-1**: a cell is now a depletable STOCK `B` with biome/season/aquatic logistic regrowth (`capacity.py`, opt-in). **GATES: 1 (Miami viability) PASS, 2 (biome sanity) PASS, 3 (aquatic subsidy) — subsidy alone does NOT concentrate bands.** R-49/R-50/R-51; MODEL_SPEC §4.3.4–§4.3.11; MECHANISMS §9b; PARAMETERS §19. Legacy bit-exact throughout (suite 660). **NEXT (the open keystone): longer SATURATION runs → the population must FILL the habitable land before concentrating on the best cells is forced (R-51: pop is demographically, not resource, limited → 0.4% land filled → IFD always disperses). Emergent sedentism → proto-ag/fishing needs CIRCUMSCRIPTION (Carneiro 1970): a bounded/saturated habitable area (or a resource rich enough that concentrating beats dispersing). Circumscription as the verification test.** Then EFC validation + cut-over from legacy. |
