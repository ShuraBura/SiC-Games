...
| morph_aquatic_gated / morph_aquatic_threshold | **False / 0.6** (candidate CANONICAL, pending sign-off) | OPT-IN | R-47: storage stays a broad survival BUFFER; only the surplus→COMPLEX morph is gated on the band's mean WATER ACCESS (`wateracc` ≥ thr) — complexity requires a storable AQUATIC resource (NW-Coast salmon; Testart/Ames). → terrestrial interiors (forest/savanna/desert) EGALITARIAN + surviving; water-rich zones (montane rivers 22%, forest 8%) complex. The anthropologically-correct pattern (complexity rare + aquatic). Off ⇒ ungated morph, bit-exact. threshold 0.6 PROVISIONAL. §4.5.10 |
