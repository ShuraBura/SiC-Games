...
| comove_anticipate / comove_footprint / comove_provision_exclude | **False / 0 / False** (default) | OPT-IN; **footprint = candidate FIX pending sign-off** | central-place co-movement (R-42): (i) root anticipates its family in its per-capita; (ii) followers scatter to low-occ cells within Chebyshev `k` of the head (dispersed camp — the load-bearing fix: footprint=2 → savanna 8→378, forest stays healthy); (iii) juvenile followers take no forage share (provisioned). All default OFF ⇒ bit-exact exact-snap. Canonical footprint radius pending supervisor. §4.8.20 |
