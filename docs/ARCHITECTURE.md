...
4. **Desert left PROVISIONAL, unchanged at 1,201.** Directive §3 required reading Bird et al. 2009 Figure 4 for per-species rates. The source (Am. Antiquity 74(1):3–29) **arrived in `literature/` mid-task** (supervisor upload 2026-06-15), but its per-species rates are **box-plot figures, not a clean table**, and cannot be read by text extraction (image rendering unavailable — no `pdftoppm`/poppler; §3.1 itself specified "page image, not OCR"). Text anchors confirm an extreme right-skewed spread (sand monitor all-seasons ≈641; bustard post-encounter **>100,000 kcal/hr** tail), which **confirms the 1,201 midpoint is indefensible** but still cannot be replaced without an image read of the figure. Tagged `pending supervisor read of Bird 2009 Fig 4`. Method when readable: encounter-weighted mean / median tail-resistance, mirroring forest.
