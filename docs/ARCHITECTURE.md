...
4. **Desert left PROVISIONAL, unchanged at 1,201.** Directive §3 required reading Bird et al. 2009 Figure 4 for per-species rates, but **no Bird et al. 2009 (Martu desert) source is available** (the `literature/` folder added 2026-06-15 has Hill 1987, Bird 1997, Alyawara, +~25 others, but no Bird 2009 Martu game paper) — per-species figures unavailable, so the midpoint cannot be replaced with a defensible species-median. Tagged `pending supervisor + Bird 2009 Fig 4`. Method when available: encounter-weighted mean, mirroring forest.
