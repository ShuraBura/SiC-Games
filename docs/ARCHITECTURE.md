...
2. mountain — DEFAULT: `elev > 0.72+(1−relief)*0.5` AND `slope > 0.18+(1−relief)*0.4`. OROGENIC (`orogenK`>0, alpine preset): above the tree-line `warmest_month air < 10 °C` (Köppen ET isotherm; elevation/lapse-driven; §9.5.1a, R-59).
3. desert (`npp < 0.10`)
...
### §12.1-M — Game-table representative-value derivations + intertidal→forage reclass + GAME_KCAL_TARGETS reconcile (2026-06-15)

**Context:** `GAME_KCAL_TARGETS` (terrain.py) previously read as picked constants — the game table stated conclusions ("mean of 7 species", "mid-range") without the arithmetic. Reconcile directive made the code follow the table.

**Decisions:**
1. **Forest representative value: pursuit-weighted mean (method LOCKED by supervisor).** Table 2 reports per-species pursuit counts `n` (encounter frequencies), so each post-encounter rate is weighted by `n` rather than flat-averaged (a flat mean of the 1,370–15,398 spread overstates typical yield). Dual-value picks for denominator-consistency: peccary 5,323 (tracking ∈ acquisition attempt, matches table footnote a); armadillo kept as two encounter modes (surface 13,782 / burrow 2,662). **Forest: 7,749 → 5,541 kcal/hr** (1,462,745/264; median cross-check 6,120). Derivation home: game table §F.2.1.
2. **Intertidal reclassified game → forage.** Intertidal shellfishing (Bliege Bird 2001, 4,653 gross / net ≈ 0) is forage, already anchored as `SHORE_BONUS_KCAL = 1491.5` on `forage_kcal`. Holding it as a game cell double-counted the same activity. Game-table row struck (provenance preserved as cross-ref); `GAME_KCAL_TARGETS` correctly has no intertidal key. Category boundary: terrestrial vertebrate prey = game; intertidal/roots/tubers/plants = forage.
3. **Savanna 518 / Grassland 3,001 unchanged**, now derivation-pointed (savanna static cell = all-seasons base; 745 dry-season intercept = seasonality hook). 
4. **Desert left PROVISIONAL, unchanged at 1,201.** Directive §3 required reading Bird et al. 2009 for per-species desert rates. The source (Am. Antiquity 74(1):3–29) arrived in `literature/` mid-task and **was read via page-image rendering** (pypdfium2; poppler absent). Key correction: the paper has **two denominators** — search-inclusive *overall hunt-type* rates (sand monitor all-seasons ≈641; the game table's "641–1,761" range) vs *post-encounter* rates (Table 2/Fig 5, thousands; bustard tail >100,000). Per §F.1 the desert cell is **search-inclusive**, so the overall hunt-type rates (641-class) are the correct basis — **not** the post-encounter thousands (an earlier draft conflated them). The 1,201 midpoint stays indefensible. Per-hunt-type overall rates now read: sand monitor 641 (n=612), perentie 765 (n=78), bustard ~1,300 (n=91, "nearly double" — qualitative); hill kangaroo (n=289) excluded (overall rate not cleanly stated), cat opportunistic. **Bout-frequency-weighted mean = 570,262/781 ≈ 730 kcal/hr** (median 765; both ≈750, vs 1,201). Proposed, NOT locked — supervisor confirms basis + bustard/kangaroo + collapse rule. Constant unchanged. Derivation: game table §F.2.1.
