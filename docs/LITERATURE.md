...
**What was lifted — the status→reproductive-success calibration target (§4.5.7, §4.8.5):** male status→RS meta-analytic **r ≈ 0.19** (modest in humans; ~equal weight across formidability/hunting/wealth/influence; the mating channel exceeds the survival channel; polygyny amplifies ~⅓). **How used:** the calibration target for `mate_choice_strength` m — corr(prowess, surviving offspring | male) is tuned to ≈0.19. **IFD substrate: m≈4** (R-19); **banded substrate: m≈5** (R-21, §4.8.5 — the band-territory mate-gate dilutes the skew, so a higher m reaches the same r). Anchors the design choice to keep the mate-choice skew LOW.
...
### Birdsell, J.B. (1953/1968). "Some environmental and cultural factors influencing the structuring of Australian Aboriginal populations" (*Am. Naturalist* 87:171) & "Some predictions for the Pleistocene based on equilibrium systems among recent hunter-gatherers" (in Lee & DeVore eds., *Man the Hunter*, 1968).
**What was lifted — the band "magic numbers" (F.3c-1, §4.8.8):** the **~25-person band** (the land-using/foraging group, the "dialectal" co-residence unit) nesting in a **~500-person dialectal tribe / connubium** (the endogamous breeding population), from Australian Aboriginal + cross-cultural equilibrium data. **How used:** the *target* emergent affiliation-band size (~25) for F.3c-1; the ~500 connubium is the next nesting level (cf. Wobst's ~475). **Status: [UNVERIFIED — values from the standard secondary literature; flagged for PDF check.]**
**Citation tag:** [TARGET — band size ~25 (§4.8.8); NEEDS PDF VERIFICATION]

### Hamilton, M.J., Milne, B.T., Walker, R.S., Burger, O., Brown, J.H. (2007). "The complex structure of hunter-gatherer social networks." *Proc. R. Soc. B* 274(1622):2195–2202.
**What was lifted — nested/self-similar social structure (F.3c, §4.8.8):** HG social organization is **hierarchically nested and self-similar (fractal)** — individual → family/household → band → "megaband"/community → tribe — with a roughly constant **scaling ratio ≈ 3–4 (≈3.8)** between successive levels (≈ 1 → 4 → 15 → 50 → 150 → 500). **How used:** the conceptual justification for an explicit BAND level that *bundles families* and itself nests in a community — motivates the collective-identity vector + the multi-family band; the ratio sets the rough level-sizes (family ~few → band ~15–25 → community ~50–150). **Status: [UNVERIFIED — scaling ratio from the standard secondary literature; flagged for PDF check.]**
**Citation tag:** [REFERENCE — nested band structure / scaling (§4.8.8); NEEDS PDF VERIFICATION]
