...
### Guyon, L. et al. (2024). "Patrilineal segmentary systems provide a peaceful explanation for the post-Neolithic Y-chromosome bottleneck." *Nature Communications* 15. (PMC11043392.) `[WEB-VERIFIED 2026-09-18 from PMC full text — PDF TO FILE, run verify_anchor.py before treating as hard]`
**What was lifted — the marker #5 RE-ANCHOR (lineage DYNAMICS, R-106 Add.98):** the size-Gini #5 is un-scoreable (foragers lack unilineal descent; no within-population lineage-size Gini exists). Re-targeted to patrilineal descent-group DYNAMICS, for which this paper gives EMPIRICAL per-generation ranges (Supplementary Tables 1–2): **extinction [0.16, 30]%/generation**, **successful-group growth [0.12, 2.5]/generation**. **CAVEAT:** the ranges are BROAD (200× extinction span) and skew DYNASTIC (Chinese imperial lineages, the Uí Néill "~20% of males"), so use as a CONSISTENCY SCREEN, not a tight target. **Measured (Add.98):** model growth 1.36×/gen (IN range), extinction 36–39%/gen (just over the 30% upper — defensible for small high-mortality forager lineages). **Rejected:** the size-Gini as a scoreable quantity.
**Citation tag:** [CANDIDATE — #5 lineage-dynamics screen (extinction/growth per generation); PDF to file]

### Page, A.E. et al. Agta forager wealth/health inequality. (PMC10237286.) `[WEB-VERIFIED 2026-09-18 from PMC full text — PDF TO FILE]`
**What was lifted — the marker #14 WITHIN-CAMP cross-check (R-106 Add.98):** Agta foragers' WITHIN-CAMP material wealth Gini = **mean 0.23 (SD 0.12, range 0–0.44)**, age-corrected, over a 10-item household-goods list, 11 camps (Table 1). This is a FINER-unit companion to BHM 2009's society-scale HG material Gini 0.36 (marker #14). **How used:** validates the model's material distribution at the within-camp (cell) scale — `material_gini_within_cell` reads ~0.27, inside the Agta range near its mean — complementing the society-scale `material_gini_adults` ≈ 0.36. Confirms foragers DO carry modest material inequality.
**Citation tag:** [CANDIDATE — #14 within-camp material-Gini cross-check 0.23 [0–0.44]; PDF to file]
