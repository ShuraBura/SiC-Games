...
**What was lifted — the band "magic numbers" (F.3c-1, §4.8.8):** the **~25-person band** (land-using/foraging local group) nesting in a **~500-person dialectal tribe / connubium** (endogamous breeding population), from Australian Aboriginal + cross-cultural equilibrium data. **How used:** the *target* emergent affiliation-band size (~25) for F.3c-1; ~500 is the next nesting level (cf. Wobst ~475; Hamilton 2007 community). **CAVEAT:** the **~500** is CONTESTED — critics ("On the magic number 500: an expostulation"; Helm) show the 1953/1968 derivation used questionable algebra + a biased sample, so 500 is a soft modal target, NOT a law. The **~25 band** is independently corroborated (Wobst 1974, Kelly, Hill 2011) and is what F.3c-1 targets. **Status: VERIFIED via web (vol/pages/DOI); PDFs not yet filed.**
**Citation tag:** [TARGET — band size ~25 (§4.8.8); the ~500 connubium is contested]
...
**What was lifted — nested/self-similar social structure (F.3c, §4.8.8):** from **1189 social groups across 339 HG societies**, HG social organization is **hierarchically nested + self-similar (fractal)** — individual→family→band→community→tribe — with a roughly constant **scaling ratio ≈ 4** ("close to 4") between levels (≈ 1→4→16→64→256). **How used:** justifies an explicit BAND level that bundles families + nests in a community — motivates the collective-identity vector + multi-family band; the ~4× ratio sets the rough level-sizes (family ~few → band ~15–30 → community ~50–150). **Status: VERIFIED via web (authors/vol/pages/DOI; ratio ≈4); PDF not yet filed.**
