...
**What was lifted — risk-dilution / safety-in-numbers (E.1 safety drive, §4.8.1; F.2-mortality prep):** aggregation lowers an individual's predation/exposure risk (each animal reduces its "domain of danger" by clustering) — the canonical selfish-herd argument for grouping. **How used:** the E.1 movement multiplier `ypc ×= 1 + s_max·(1 − e^{−g/g_s})` — a saturating per-capita safety benefit of band size `g`, traded against the falling per-capita yield so an optimal band size emerges. Forthcoming F.2 wires the same dilution into the *mortality schedule* (safety as a survival benefit, not just a movement preference).
**Citation tag:** [USED — E.1 grouping safety drive (§4.8.1); risk-dilution mortality (F.2, pending)]
