...
| 11 | **status → RS** | `status_rs_r_partial` (was `status_rs_r`) | 0.15 monogamous / 0.19 cross-system | von Rueden & Jaeggi | **MET on an 800-step probe (R-106 Add.94: 0.157 temp / 0.143 sav) — DIVERGES on the full 15,000-step canonical run (R-106 Add.100): 0.283 temp / 0.141 sav / 0.262 montane. Savanna still matches; temperate and montane run ~2× the anchor and do NOT track `gini_cred` cleanly. NOT DIAGNOSED — open thread, flagged for Opus-level review.** |
...
| 17 | **fission ceiling** | `settle_community_max` (was `settle_max`) | communities should not persist far past **158 [147–170]** (max scalar stress) and effectively never past **250** (ethnographic maximum) | Alberti 2014 `[VERIFIED VERBATIM]` + Alvard 2009 `[VERIFIED VERBATIM]`; Hamilton 2007 periodic aggregation **165.32 [152.25–181.00]** `[VERIFIED VERBATIM]` | **AT THE CEILING on an 800-step probe (R-106 Add.93: 155 temp / 200 sav) — PARTIALLY REOPENS on the full 15,000-step canonical run (R-106 Add.100): savanna still lands at 155.1, but temperate (255.7) and montane (301.6) now sit OVER the 250 ceiling. May be genuine long-run drift, or the documented max-grows-with-samples confound (601 snapshots vs ~15). NOT DIAGNOSED — open thread, flagged for Opus-level review.** |
