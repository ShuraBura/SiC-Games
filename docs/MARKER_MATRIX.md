...
| 17 | **fission ceiling** | `settle_community_max` (was `settle_max`) | communities should not persist far past **158 [147–170]** (max scalar stress) and effectively never past **250** (ethnographic maximum) | Alberti 2014 `[VERIFIED VERBATIM]` + Alvard 2009 `[VERIFIED VERBATIM]`; Hamilton 2007 periodic aggregation **165.32 [152.25–181.00]** `[VERIFIED VERBATIM]` | **SCORE AGAINST `village_max`, NOT `settle_community_max` (R-106 Add.103).** Add.93's 3×3-window marker is a WINDOW, not a partition: at canonical scale the densest window holds 307 people spanning **3.0 villages and 40.7 band_ids** (largest single band inside it: 69). It straddles wherever villages are small AND packed (temperate, window÷village = 1.33; savanna/montane ≈ 1.0). `LITERATURE` fixes the anchor's unit — "Alberti's N≈127 is village/settlement scale" — so use the Voronoi partition `village_max`: **temperate 192.5 (past 158, under the 250 ceiling), savanna 154.1 (ON Alberti 158), montane 288.0 (GENUINE over-run)**. So the Add.100/101 "temperate+montane over-run" reduces to MONTANE ALONE; temperate's breach was the window summing three villages. Montane's excess looks terrain-funnelled (largest villages, 9.8 cells, at the lowest per-cell density) — hypothesis, untested. Also density-sensitive (1.65×, Add.101). |
