...
|---|---|---|---|---|---|
| 1 | band size | `band_med` → mean-experienced per CELL | **28.2 ADULTS** (Hill 2011, 32 societies) — the PERSON-WEIGHTED "mean experienced" adult size on a RESIDENTIAL group | Hill et al. 2011 `[VERIFIED, PDF read]`; ~~Johnson~~ | **NO DEFICIT — RE-SCORED (R-106 Add.89, 2026-09-16).** The old "FAILS 16/16 at 0.42×" used the MEDIAN over `band_id` — wrong statistic (Hill is person-weighted mean-experienced) AND wrong unit (`band_id` = ~8-cell affiliation, not a camp; the model's co-residence unit is the CELL). Correct statistic + cell unit → **33 temperate / 23 savanna ≈ 28.2**, inside the per-society range ~12–40 |
...
| 14 | **wealth concentration** | `material_gini_adults` (was `material_gini`) | HG **0.36** / hort 0.52 / pastoral 0.51 / agric 0.57 (BHM Table S5, material column, **age-adjusted ADULTS**) | BHM 2009 (T-5) | **OPEN — 0.16–0.22 on the corrected ADULTS statistic (R-106 Add.90).** Add.87 adopted grounded concentrators and the ALL-AGES `material_gini` reached 0.36 — but that is child-inflated; BHM is age-adjusted adults. On `material_gini_adults` the adopted canon is 0.16–0.22 vs 0.36 (concentrators gave partial adult concentration 0.13→0.19). The old marker was the wrong statistic |
