...
2b. **A WINDOW IS NOT A PARTITION** (R-106 Addendum 103 — a sharpening of the UNIT rule). Any "largest local group"
   measure built from a sliding window over-counts wherever the groups it means to separate sit closer together than
   the window is wide, and the error is invisible in the number because it scales with how packed the landscape is,
   not with the groups. Measured: #17's 3×3 window holds 307 people spanning 3.0 villages and 40.7 band_ids.
   Prefer a PARTITION — every individual assigned exactly once (`village_sizes()` nearest-site Voronoi). Third time
   a spatial unit has bitten this project: band_id-vs-cell (Add.89), settlement double-count (Add.95), window-vs-
   partition (Add.103).
...
| 17 | **fission ceiling** | `settle_max` (exact-cell co-residence — the ORIGINAL field; Add.93's `settle_community_max` "fix" was the error) | communities should not persist far past **158 [147–170]** (max scalar stress) and effectively never past **250** (ethnographic maximum) | Alberti 2014 `[VERIFIED VERBATIM]` + Alvard 2009 `[VERIFIED VERBATIM]`; Hamilton 2007 periodic aggregation **165.32 [152.25–181.00]** `[VERIFIED VERBATIM]` | **PASSES on the co-residence unit, all three worlds (R-106 Add.104).** The largest genuinely co-resident group is **108 temperate / 85 savanna / 103 montane** — all far under Alberti's 158. The cell is the model's co-residence unit (Add.89) and at 100 km² is an UPPER BOUND on one settlement, so no canonical world breaches the scalar-stress ceiling with anything people actually live in. Every apparent over-run came from coarser bookkeeping: cell 103–115 → Voronoi catchment 192/154/288 → 3×3 window 256/155/302 → `band_id` 439/404/1080. Montane's 288 catchment is NOT a good-spot effect (its forage Gini 0.241 ≈ temperate 0.220, occupied/empty ratio 1.41× in both, 82.5% of forage left unused) — it holds the same footprint with HALF the sites (23 vs 47), so each catchment sweeps 9.1 cells not 4.3. Details below. |
**#17 measurement history (superseded, kept for the trail).** Add.93 introduced the 3×3 `settle_community_max`; Add.103 showed a WINDOW IS NOT A PARTITION (the densest window holds 307 people spanning 3.0 villages / 40.7 band_ids, largest single band inside it 69) and moved scoring to the Voronoi `village_max`; Add.104 then showed even that catchment inflates where sites are sparse, and settled scoring on the co-residence CELL. Density-sensitive throughout (1.65×, Add.101), so state the density.
