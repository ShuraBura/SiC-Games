...
| 11 | **status → RS** | `status_rs_r_partial` (was `status_rs_r`) | 0.15 monogamous / 0.19 cross-system | von Rueden & Jaeggi | **MISS on canon — REOPENED (R-106 Add.101).** Add.94's "MET" (0.157/0.143) was measured on the probe harness, which builds a DIFFERENT, lower-density world (capacity patch (20,20,24) vs canon's (30,30,40)). On the canonical campaign: **0.283 temp / 0.141 sav / 0.262 montane** vs von Rueden 0.15 — temperate and montane ~2× the anchor. The marker is strongly DENSITY-SENSITIVE (2.29× across an 8× population range), so like #4 it must be scored at a STATED DENSITY, never pooled. Campaign values are authoritative (`gen_runconfig` derives canon by running it). |
