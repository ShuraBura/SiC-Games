...
The current constant **1,201** is the bare midpoint of the Bird et al. 2009 (Martu) 641–1,761 species range. Reconcile directive §3 instructed reading **Bird et al. 2009 Figure 4** for the per-species post-encounter rates, to apply the same encounter-weighted method as forest. **The source — Bird, Bliege Bird & Codding 2009, "In Pursuit of Mobile Prey," American Antiquity 74(1):3–29 — is now present** in `literature/` (added by the supervisor 2026-06-15, mid-task). However, the per-species post-encounter rates are presented as **box-plot Figures (percentile boxes), not a clean numeric table**, and reliable per-species values cannot be read by text extraction (the figure must be read as a page **image**; `pdftoppm`/poppler is not installed in this environment, which is exactly why §3.1 specified "page image, not OCR"). Text-extractable anchors confirm an **extreme spread**: sand monitor all-seasons average **≈ 641 kcal/hr** (the low end of the 641–1,761 range); hill kangaroo in the low thousands; **bustard post-encounter rate is a massive tail — "over 100,000 kcal/hr"** for some pursuits (five points beyond 60,000, per Fig). This **confirms the 1,201 midpoint is indefensible** — the true distribution runs ≈ 641 → 100,000+ and is wildly right-skewed, so a midpoint of any sub-range overstates typical yield and the bustard tail dominates any flat mean. A defensible value needs the figure's per-species rates **and** encounter frequencies, read from the image, then the same encounter-weighted-mean / median tail-resistance as forest. Desert therefore stays at 1,201 tagged `[PROVISIONAL — desert method pending supervisor read of Bird 2009 Fig 4 (image rendering unavailable here)]`; **the code constant is not changed** (§3.4: supervisor confirms before any change). O'Connell & Hawkes 1984 (Alyawara sandplain ~3,200 / mulga ~650; `Alyawara_plant_use_and_optimal_foraging.pdf` now in `literature/`) usable only as a patch-level sanity band, never an override of the Martu species-resolved value.
