...
def compute_metrics(
    ...
    if phi_vals:
        ...
        # Compute W once; reuse for all four traits (W depends only on positions).
        _W = _moran_W(positions, sugar_field.width, sugar_field.height)
        mi_phi = morans_i(phi_vals, positions, sugar_field.width, sugar_field.height, W=_W)
        mi_psi = morans_i(psi_vals, positions, sugar_field.width, sugar_field.height, W=_W)
        mi_c1  = morans_i(c1_vals,  positions, sugar_field.width, sugar_field.height, W=_W)
        mi_c2  = morans_i(c2_vals,  positions, sugar_field.width, sugar_field.height, W=_W)
    else:
