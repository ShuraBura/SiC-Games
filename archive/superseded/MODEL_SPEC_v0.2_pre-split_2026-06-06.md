# SiC Games — Model Specification (MODEL_SPEC.md)

**Status:** FULL EXTRACTION (v0.2) — covers all mechanisms through Stage 5.2.
**Purpose:** The single authoritative reference for *what each construct in the model is, what real-world dynamic it represents, where it came from in the literature, how SiC departs from that precedent, its support/range, its inheritance channel, and its lock status.* This is the document you consult instead of grepping the stage blueprints. It is an ODD-style construct registry (Grimm et al. 2006/2020) adapted to this project.
**Maintainer protocol:** Updated whenever a construct is introduced, redefined, or has its lock status changed. Reviewed at the start of every stage conversation, like `ROADMAP.md`. Where this document and a stage blueprint disagree, **the blueprint is the historical record and this document is the current truth** — reconcile and log the discrepancy in §15.
...
> **v0.2 SCOPE.** This extraction extends the pilot (v0.1) to cover: the resource substrate (§9), the decision/σ economy (§3), the joint-task + Matthew partition (§4), C and Si Cred (§5), the support pool L1–L3 (§6), reproduction + demography (§7), shocks/perturbations (§8), selection measurement (§10), metrics & diagnostics (§11), and the parameter registry pointer (§14). §12 design-decisions log, §13 architecture seams, §15 gaps carry forward and extend the pilot. §0–§4 (pilot content) renumber accordingly; all pilot content is preserved verbatim in its sections except for section-number relabelling.
...
A fourth status, **SEAM**, is orthogonal: a documented architectural joint where a *deferred* mechanism will plug in, present in the code as an inert (λ=0 / `NotImplementedError`) placeholder under an equivalence gate. Seams are cheap to place during active development of the host subsystem and ruinous to retrofit into locked, tested code; see §13.
...
**ψ Deffuant note.** Updating ψ via Deffuant in Stage 5.2 was *deliberate*, not accidental: the Stage 5.2 blueprint (§3.2) states it is "intentional and is the homogenising force tested against in Task 3." The confusion that motivated this document arose not from the bundling itself but from the Stage 5.2 *report* then gating σ_inherit on ψ diversity while ψ was simultaneously subject to that homogenising force. See §3.3 and the design-log entry §12.1-D.
...
**Status: PROPOSED, not implemented.** See design-log §12.1-A.

### 2.3 ψ's home — cultural or physical channel? (OPEN)

*(Pilot §2.3 preserved verbatim.)*

ψ (sociability) sits awkwardly between the cultural and physical channels. Moving ψ to physical-vertical-only would remove it from the Deffuant contracting force entirely, dissolving the Cell B tension. Status: **OPEN.** See §15 open decisions.
...
| Coordinator | individual; hivemind = **SEAM** (Si only, NotImplementedError) | individual | **C3 seam** | Stage 4.1a §1.4; §13 |
...
**σ_inherit note (§12.1-D):** the σ_inherit lock at 0.10 (Stage 5.2) is under review. The Stage 5.2 Task 3 selection gate used Gini(ψ) — the wrong statistic for a bounded [0,1] trait (use SD) and the wrong target trait (c1/c2 carry the theory, not ψ). σ_inherit=0.10 should be treated as **OPEN/under review** pending a corrective directive targeting c1/c2 diversity. See §12.1-D.
...
movement-instrumentation stage. Feeds H-ORTHOGONALITY test and MODEL_SPEC §3 C2
ψ-channel resolution.
...
Dispersion statistic note (§12.1-D): for bounded [0,1] traits initialised at mean 0.5 / SD 0.2, **use SD, not Gini**. Gini is mean-sensitive and ill-behaved near a point mass. A collapse to SD < ~0.05 (from SD₀=0.2) is the meaningful homogenisation signal.
...
**Status:** not implemented. Mixing-rule sub-decision (biparental-average vs single-parent-copy for the physical channel) OPEN — see §15.
...
**Authoritative parameter values + lock/sweep history: `ROADMAP.md` locked-parameter table** (to migrate to `PARAMETERS.md` when that document is built — see pending directive). Do NOT copy parameter values into this section; that would create two-homes drift.
...
The following citations appear in blueprints but are NOT in LITERATURE.md (which covers only Stage 5 Si Cred). They enter as **`[INLINE]`** and must be added to LITERATURE.md before use in any write-up:

| Citation | Mechanism | Blueprint(s) | Current tag |
|---|---|---|---|
| Deffuant et al. (2000), *Mixing beliefs among interacting agents* | Deffuant bounded-confidence updating (§3.3) | Stage 3.3 §0; 5.2 §3 | `[INLINE]` |
...
|---|---|---|
| Richerson & Boyd (2005), *Not by Genes Alone* | Dual inheritance theory | §2.2 |
| Price (1970, 1972); Frank (1995/1997/2012) | Price-equation selection decomposition | §10 |
| Grimm et al. (2006, 2010, 2020) | ODD protocol | Header |
| Merton (1968) | Matthew Effect (cumulative advantage) | §4.2 |
...
1. **ψ's channel** — cultural (Deffuant-subject) vs physical (vertical-only disposition). §2.3. Resolving toward physical dissolves the Cell B tension.
2. **Physical-channel mixing rule** — biparental-average (C) vs single-parent-copy (Si) for physical inheritance. §2.2.
...
ψ, c1, c2 are bounded `[0,1]` traits initialised at mean 0.5 with SD 0.2. **Gini is the wrong dispersion measure** for a bounded, mean-0.5 trait. Use **SD (or variance)** referenced to the initial SD=0.2; a collapse to SD < ~0.05 is a meaningful homogenisation signal. The Stage 5.2 report's Gini(ψ) ≥ 0.15 floor was arbitrary partly *because* it was the wrong statistic.
...
*End of MODEL_SPEC.md — v0.2, full extraction 2026-05-29.*
